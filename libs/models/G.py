...
def define_G(configs):
    ...
    elif configs.name == 'style_translator2':
        net = StyleTranslator2(**configs.params)
    else:
...
class StyleTranslator2(nn.Module):
    ...
    def __init__(self,
        ...
        super(StyleTranslator2, self).__init__()
        ...
        for i in range(encoder1_blocks):
            ...
            encoder1.append(
                ConvNormAct(
                    in_dims=in_dims, out_dims=out_dims,
                    conv_type='conv2d', kernel_size=3, stride=1,
                    padding=1, bias=use_bias, norm_layer=norm_layer,
                    sampling='down'
                )
            )
