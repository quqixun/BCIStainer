...
def define_G(configs):
    ...
    elif configs.name == 'style_translator_ocahr':
        net = StyleTranslatorOriCAHR(**configs.params)
    else:
...
class StyleTranslatorOriCAHR(nn.Module):
    ...
    def __init__(self,
        ...
        super(StyleTranslatorOriCAHR, self).__init__()

        self.full_size  = full_size
        self.crop_size  = crop_size
        self.full_shape = (full_size,) * 2
        self.crop_shape = (crop_size,) * 2
    ...
    def _forward_full(self, he, crop_idxs):
        ...
        he_ = interpolate(he, self.crop_shape)
        he_in = self.inconv(he_)
        enc1  = self.encoder1(he_in)
        style = self.encoder2(enc1)
        level = self.classify_head(style)
        ...
        dec2 = self.decoder2(dec1)
        ihc_full = self.highres_outconv(dec2)
        ihc_full = interpolate(ihc_full, self.full_shape)

        if self.mask_dec_input == 'dec1':
            mask_dec = self.mask_decoder(dec1)
        elif self.mask_dec_input == 'enc1':
            mask_dec = self.mask_decoder(enc1)
        mask_full = self.mask_outconv(mask_dec)
        ...
        for i in range(mask_full.size(0)):
            row1, col1 = crop_idxs[i]
            row2, col2 = crop_idxs[i] + self.crop_size
            mask_full_mask[i, :, row1:row2, col1:col2] = 1.0
        mask_full = mask_full * mask_full_mask
        ...
        outputs_dict['level']     = level
        outputs_dict['ihc_full']  = ihc_full
        outputs_dict['mask_full'] = mask_full
        return outputs_dict
    ...
    def _forward_crop(self, he_crop, style):
        ...
        outputs_dict = {'ihc_crop': ihc_crop}
        return outputs_dict

    def _train_merge(self, ihc_full, ihc_crop, mask_full, crop_idxs):
        ...
        for i in range(ihc_full.size(0)):
            ...
            ihc_crop_pad = F.pad(ihc_crop[i], col_pad + row_pad)
            ihc_full_ = ihc_full[i] * (1 - mask_full[i])
            ihc_crop_ = ihc_crop_pad * mask_full[i]
            ihc_hr_   = ihc_full_ + ihc_crop_
            ihc_hr_list.append(ihc_hr_)
    ...
    def _infer_full_merge(self, ihc_full, ihc_crop, mask_full, crop_idxs):
        ...
        for i in range(ihc_crop.size(0)):
            ...
            ihc_crop_pad = F.pad(ihc_crop[i], col_pad + row_pad)
            ihc_full_ = ihc_full * (1 - mask_full[i])
            ihc_crop_ = ihc_crop_pad * mask_full[i]
            ihc_hr_   = ihc_full_ + ihc_crop_
            ihc_hr   += ihc_hr_
    ...
    def _infer_crop_merge(self, ihc_full, ihc_crop, mask_full, crop_idxs):
        assert ihc_full.size(0) == 1
        ...
        for i in range(ihc_crop.size(0)):
            ...
            ihc_full_crop_  = ihc_full[:, row1:row2, col1:col2]
            mask_full_crop_ = mask_full[i, :, row1:row2, col1:col2]
            ihc_full_crop_  = ihc_full_crop_ * (1 - mask_full_crop_)
            ihc_crop_ = ihc_crop[i] * mask_full_crop_
            ihc_hr_   = ihc_full_crop_ + ihc_crop_
    ...
    def forward(self, he, he_crop, crop_idxs, mode):
        ...
        full_outputs = self._forward_full(he, crop_idxs)
        level     = full_outputs['level']
        ihc_full  = full_outputs['ihc_full']
        mask_full = full_outputs['mask_full']
        style     = full_outputs.get('style', None)
        ihc_lr    = full_outputs.get('ihc_lr', None)
        ...
        ihc_hr = merge_func(ihc_full, ihc_crop, mask_full, crop_idxs)
